...
class ParticleFilter(InferenceModule):
	...
	def initializeUniformly(self, gameState):
		...
		"*** YOUR CODE HERE ***"
		self.particles = []
		totalParticles = range(self.numParticles)
		legalPosition = self.legalPositions

		for p in totalParticles:
			distribution = p % len(legalPosition)
			self.particles.append(self.legalPositions[distribution])
	...
	def observe(self, observation, gameState):
		...
		if noisyDistance == None:
			...
			weight = util.Counter()
			for p in self.legalPositions:
				h = util.manhattanDistance(p, pacmanPosition)
				bust = emissionModel[h] * b[p]
				weight[p] += bust
...
class JointParticleFilter:
	...
	def initializeParticles(self):
		...
		c = 0
		self.particles = []
		tupPos = list(itertools.product(self.legalPositions, repeat = self.numGhosts))
		random.shuffle(tupPos)
	...
	def observeState(self, gameState):
		...
		"*** YOUR CODE HERE ***"
		weight = util.Counter()
		totalGhosts = range(self.numGhosts)
		totalP = range(0, self.numParticles)
		for p, particle in enumerate(self.particles):
			w = 1.0
			for x in totalGhosts:
				if noisyDistances[x] == None:
					particle = self.getParticleWithGhostInJail(particle, x)
				else:
					h = util.manhattanDistance(particle[x], pacmanPosition)
					w *= emissionModels[x][h]
			weight[particle] += w
		if not any(weight.values()):
			self.initializeParticles()
			for p, particle in enumerate(self.particles):
				for i in totalGhosts:
